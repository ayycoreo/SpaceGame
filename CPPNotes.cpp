...
void vectors(void) {
    ...
    myVector.empty(); // returns true if the vector is empty
...
// Makes adding elements faster by allocating memory in advance
    ...
    std::count(myVector.begin(), myVector.end(), 3); // counts the number of occurrences of 3 in the vector
...
void stdLists(void)
{
    ...
    */
    ...
    a.splice(pos, b); // Moves all elements from b into a before 2

    // Now: a = {1, 4, 5, 6, 2, 3}, b is empty


    std::list<int> a = {1, 2, 3};
    std::list<int> b = {4, 5, 6};
    auto pos = a.end();
    auto it = b.begin(); // points to 4
    a.splice(pos, b, it); // a = {1, 2, 3, 4}, b = {5, 6}

    std::list<int> a = {1, 2, 3};
    std::list<int> b = {4, 5, 6, 7};
    auto pos = a.begin();
    auto first = ++b.begin(); // points to 5
    auto last = b.end();      // points past 7
    a.splice(pos, b, first, last); // a = {5, 6, 7, 1, 2, 3}, b = {4}
...
void stdDeques(void)
{
    ...
    myDeque.size(); // returns the number of elements in the deque
    myDeque.empty(); // returns true if the deque is empty
}
...
void stdSets(void)
{
    ...
    mySet.empty(); // returns true if the set is empty
    ...
    */
    ...
    auto it2 = s.lower_bound(3); // points to 3
    ...
    */
    std::set<int> s = {1, 3, 5, 7};
    auto x = s.upper_bound(5); // points to 7
    auto y = s.upper_bound(7); // points to end()
...
void stdMaps(void)
{
    ...
    myMap.empty(); // returns true if the map is empty
